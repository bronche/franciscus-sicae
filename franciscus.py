...
# Affichage historique cliquable depuis CSV
try:
    historique_df = pd.read_csv("historique_questions.csv", names=["datetime", "sujet", "question"], encoding="utf-8")
    with st.expander("📂 Questions précédentes (clic pour réutiliser)"):
        for index, row in historique_df[::-1].iterrows():
            if st.button(f"🕘 {row['question']}", key=f"hist_{index}"):
                st.session_state.question_predefinie = row["question"]
